...
The recommended way to install QuickCal is via [Package Control][package-control].  Package Control will install the
...
/// warning | Warning
This is not the recommended way to install QuickCal for the average user.  Installing this way **will not** get
automatically updated.

If you are forking for a pull request, you should **just** clone QuickCal and run Package Control's `Satisfy Dependency`
command to get all the dependencies.
///

For those who want to install QuickCal without package control, here are the steps.  It is understood that some people,
for what ever reason, will prefer manual install and may even have legitimate reasons to do so.  When going this route,
...
2.  Download and unpack, or git clone, the latest QuickCal release and unpack as `QuickCal`:
    ...
    -   @facelessuser/QuickCal --> QuickCal
