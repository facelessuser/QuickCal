...
2.  When an issue is created, fill out with copy/pastable minimal working examples. Issues that do not provide enough
    information will be closed as invalid, especially if additional requests to provide such information are ignored.

3.  When creating an issue on the repository, please provide as much info as possible:

    -   Provide environment information by running `Preferences->Package Settings->QuickCal->Support Info`.  The
